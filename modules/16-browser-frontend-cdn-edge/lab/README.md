...
Before measured evidence, run
`python3 host_browser_callback.py --output wsl-browser-callback.json` inside WSL
and open its one-time URL in the normal Windows browser. Then run the root
preflight with `--module M16 --wsl-browser-callback
